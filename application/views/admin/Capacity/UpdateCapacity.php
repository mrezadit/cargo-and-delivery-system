        ...
        <div class="container-fluid py-1 px-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                    ...
                    <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Cargo</li>
                </ol>
                <h6 class="font-weight-bolder mb-0">Update Cargo Data</h6>
            </nav>
        ...
        <div class="card-header card-header-primary">
            <div class="row">
                <div class="col-7 align-items-center pl-4">
                    <h4 class="mb-0">Update Schedule Cargo</h4>
                    <span class="text-sm mb-0 text-end">Form to update cargo data</span>
                </div>
            </div>
            <div class="d-flex pt-4" method="post">
                <div class="col-8">
                    <div class="card border-0 d-flex p-4 pt-0 mb-2 bg-gray-100">
                    <form class="pt-4" action="<?= site_url('admin/updateCapacity'); ?>" method="post">
                        <div class="row d-flex">
                            <div class="col-6">
                            <span>Schedule</span></br>
                            <div class="input-group input-group-dynamic mb-4">
                                <label class="form-label"></label>
                                <input type="hidden" name="id_capacity" value="<?= $detail['id_capacity']; ?>">
                                <input type="date" name="schedule" value="<?= $detail['schedule']?>" class="form-control">
                            </div>
                            </div>
                        </div>
                            ...
                            <div class="col-5">
                                ...
                                <select name="id_truck" data-style="btn btn-link" data-live-search="true" style="width: 100%;" required>
                                    ...
                                    <?php if (!empty($truck)) : $i = 1; foreach ($truck as $value) : ?>
                                        <option value="<?= $value->id_truck; ?>" <?= $detail['id_truck'] === $value->id_truck ? 'selected' : ''; ?>><?= $value->truck; ?></option> 
                            ...
                            <div class="col-5 pl-5">
                                ...
                                <select name="id_driver" data-style="btn btn-link" data-live-search="true" style="width: 100%;" required>
                                    ...
                                    <?php if (!empty($driver)) : $i = 1; foreach ($driver as $value) : ?>
                                        <option value="<?= $value->id_driver; ?>" <?= $detail['id_driver'] === $value->id_driver ? 'selected' : ''; ?>><?= $value->driver; ?></option> 
                                    <?php endforeach; endif; ?>
                                </select>
                                </span>
                            </div>
