    ...
    <div class="row mb-3">
          ...
          <div class="card-body p-3">
            ...
            <div class="row">
                ...
                <div class="pr-3">
                <a href="<?= site_url('driver/completeShipping/'.$capacity['id_capacity'].''); ?>" class="badge btn-dark mb-0">Complete <i class="material-icons text-sm">open_in_new</i></a>
                </div>
            </div>
            </div>
            </div>
            ...
            <div class="card-body p-3">
                ...
                <div class="table-responsive p-0">
                    ...
                    <?php if (!empty($package)) : $i = 1; foreach (array_reverse($package) as $value) : ?>
                        ...
                        <a href="<?= site_url('driver/receiveShipping/'.$value->id_capacity.'/'.$value->id_package.''); ?>" rel="tooltip" title="detail"  class="badge btn-info btn-sm mb-0">
                        Received</a>
                        <a href="<?= site_url('driver/returnShipping/'.$value->id_capacity.'/'.$value->id_package.''); ?>" rel="tooltip" title="detail"  class="badge btn-warning btn-sm mb-0">
                        Return</a>
...
$(document).ready(function(){ // Ketika halaman sudah siap (sudah selesai di load)
    ...
    $("#date").change(function(){ // Ketika user mengganti atau memilih data provinsi
        ...
        $.ajax({
            ...
            url: "<?php echo base_url("index.php/driver/listKota"); ?>", // Isi dengan url/path file php yang dituju
    ...
    $("#kota").change(function(){ // Ketika user mengganti atau memilih data provinsi
        ...
        $.ajax({
            type: "POST", // Method pengiriman data bisa dengan GET atau POST
            url: "<?php echo base_url("index.php/driver/listPackage"); ?>", // Isi dengan url/path file php yang dituju
